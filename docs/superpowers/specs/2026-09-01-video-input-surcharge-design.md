...
为视频生成任务增加"输入参考视频按秒附加费"：管理员在页面上按模型配置一个每秒单价
（如 0.16 元/秒）。提交请求时，网关探测每个参考视频（`video_url`）的实际时长，
按 `Σ ceil(秒数) × 单价` 计入预扣与结算。该附加费是**加法项**，叠加在现有
`VideoResolutionPrice` 按秒分辨率计费之上，不改变现有乘法倍率体系。
...
{
  "seedance-2-5": 0.16,
  "doubao-seedance-2-0-260128": 0.16
}
```

（键是**计费用的 origin 模型名**——sora 通道挂的二道贩子模型名如 `seedance-2-5`
直接照填；后续 doubao 直连通道也用同一份配置。）
